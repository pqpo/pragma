...
export function migrateEpisodicJobsV2ToV3(database: DatabaseSync): void {
  const rows = database
  ...
  try {
    ...
    for (const row of rows) {
      const previous = EpisodicExtractionJobV2Schema.parse(JSON.parse(row.jobJson));
      ...
      const migrated = EpisodicExtractionJobV3Schema.parse({
        ...previous,
    ...
    database.exec(
      "CREATE UNIQUE INDEX jobs_conversation ON jobs(conversation_key); PRAGMA user_version = 3; COMMIT;",
    );
  } catch (error) {
    try {
