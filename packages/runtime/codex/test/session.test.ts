...
describe("Codex tool event mapping", () => {
  ...
  it.each([
    ["continue_expert", undefined, "resume", "followup"],
    ["steer_expert", undefined, "send", "steer"],
    ["sendInput", false, "send", "followup"],
    ["sendInput", true, "send", "steer"],
  ] as const)(
    "maps %s delivery semantics onto agent commands",
    (tool, interrupt, action, delivery) => {
      const context = {
        runId: "root-run",
        source: { kind: "runtime", runId: "root-run", path: [] },
        events: {},
      ...
      expect(result.events).toEqual([
        expect.objectContaining({
          type: "agent.command",
          payload: expect.objectContaining({ action, delivery }),
        }),
      ]);
    },
  );
