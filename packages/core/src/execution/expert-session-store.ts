...
import { migrateQueueSteerDeliveryAttempts } from "../storage/migrations/expert-session/steps/queue-steer-delivery.ts";
...
async function prepareExpertSession(
  ...
  try {
    ...
    const purposeMigratedPrompts =
      upgraded.fromVersion <= 5
        ? migratePromptPurposes(prompts as readonly unknown[], events)
        : prompts;
    const migratedPrompts =
      upgraded.fromVersion <= 6
        ? migrateQueueSteerDeliveryAttempts(purposeMigratedPrompts as readonly unknown[])
        : purposeMigratedPrompts;
