...
export interface CreateWorkflowToolsMcpServerOptions {
  ...
  readonly state: WorkflowToolRuntimeState;
...
export async function createWorkflowToolsMcpServer(
  options: CreateWorkflowToolsMcpServerOptions,
): Promise<WorkflowToolsMcpServer> {
  ...
  const tools = createWorkflowLocalTools(options);
  ...
  const httpServer = createServer((request, response) => {
    handleMcpHttpRequest(transport, request, response).catch((error: unknown) => {
      options.logger.error("Workflow MCP HTTP request failed", { error });
  ...
  if (!isAddressInfo(address)) {
    ...
    throw new Error("Workflow MCP server did not bind to a TCP address.");
  }
...
function createWorkflowLocalTools(
  options: CreateWorkflowToolsMcpServerOptions,
  ...
  const resolved = resolveToolPolicy<LocalTool>({
    ...
    tools: [
      ...
      ...createWorkflowMcpTools(options.mcpTools ?? []).map((tool) =>
...
function createWorkflowMcpTools(
