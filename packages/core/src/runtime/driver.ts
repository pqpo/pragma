...
async function createManagedRuntimeSession<TNativeEvent, TNativeSession, TPrepared>(
  ...
  const paths = createRuntimePaths(agent.workspace, descriptor);
...
function createRuntimePaths(workspace: string, descriptor: RuntimeAdapterDescriptor): RuntimePaths {
  return {
    runtimeSessionDir(runtimeName = descriptor.kind) {
      return join(workspace, ".pragma", "runtime-sessions", runtimeName, descriptor.id);
    },
  };
}
