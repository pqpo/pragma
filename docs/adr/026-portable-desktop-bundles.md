...
`pragma.bundle-installation/v1` record. Its state is one of `installing`, `needs_setup`, `ready`, or
`failed`. The source archive is retained while setup or recovery is required and removed after the
installation becomes ready. Installation records and retained archives are recoverable state under
`~/.pragma/state/bundle-installations/`; bundle payloads are not authoritative application data.
...
2. Require an explicit `update` or `copy` decision when identities or normalized names conflict.
...
`copy` allocates new IDs for the entire imported closure and rewrites typed DSL identity and
reference fields. Opaque plugin config, Flow input values, prompts, and JSON Schema content are
never searched and replaced.
`update` retains matching local identities, including name matches, and rewrites references to those
identities.
