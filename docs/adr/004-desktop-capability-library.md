...
`@pragma/core` owns runtime-neutral MCP configuration, MCP discovery/calls, SSE transport support, and the HTTP-to-MCP adapter. Desktop owns file selection, persistence, encrypted credentials, health reporting, and resolving Expert references before `ExpertAgent.create()`.
...
- HTTP services do not open a port per service. PI consumes managed tools directly; Codex and Claude Code receive them through the existing loopback workflow MCP server.
