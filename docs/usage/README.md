# Pragma 使用文档

建议按以下顺序阅读：

1. [Expert 与 ExpertSession](./agents.md)
2. [Flow](./flows.md)
3. [Flow 范式](./flow-patterns.md)
4. [Context](./context.md)
5. [Memory System](./memory.md)
6. [Plugins](./plugins.md)
7. [本地存储 v3 AI 迁移手册](../migrations/local-storage-v3-ai-migration.md)
8. [Pragma 桌面安装包](./desktop-distribution.md)
9. [Pragma CLI 人类指南：Mission 控制](./cli.md)
10. [Pragma CLI Agent / automation guide](./cli-agent.md)
11. [Pragma CLI 分发与发布运维手册](./cli-release.md)

核心架构见 [Agent Core Architecture](../architecture/agent-core-architecture.md) 与
[Execution SDK Design](../Execution-SDK-Design.md)。桌面安装包与后续在线更新规划见
[Pragma 桌面发行方案](../architecture/desktop-release-and-online-update.md)。
