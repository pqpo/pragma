...
`PromptRequest` 继续保存纯文本；附件本身不存入 ExpertSession。`pragma.expert-session/v7` 会把历史
queue-steer marker 原子迁移为显式 delivery attempt，并与 `prompts.json` 一起升级。所有新建
`expert-turn` 的 `ExecutionRecord.input` 和根 `Invocation.input` 都保存 `ExpertPromptInput`，即使没有附件也写入
`{ text, attachments: [] }`；`pragma.execution/v11` 会把历史字符串根输入迁移为这一结构。这样业务读取只处理
当前格式，崩溃恢复也能直接从根 Invocation 取得附件。
