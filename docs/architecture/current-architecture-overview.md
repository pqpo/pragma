...
Capability 使用不可变 revision、独立凭据和工具快照。Expert、System Expert 与 Project binding 只引用
Capability ID 和 tool allowlist；ready revision 由 Desktop coordinator 激活，不重写依赖资源。历史 Project
Revision 或 Mission 中的旧 revision 后缀只用于兼容读取，恢复或继续执行时仍解析当前 active revision；
Execution 保存本次实际解析的 revision 与 fingerprint。Capability 创建、删除和凭据切换也使用可恢复
journal；候选 Secret generation 只在对应 Capability 写入持久化后才切换为 active。
