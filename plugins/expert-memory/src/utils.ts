...
export function defaultTriggerForContextId(
  id: string,
): ExpertAgentContextItemMetadata["trigger"] {
...
export function calculateLineRange(content: string, startOffset: number, endOffset: number): {
  ...
  for (const line of lines) {
    if (line.startsWith("## ")) {
      if (currentHeading !== undefined) {
        sections.set(currentHeading, buffer.filter((value) => value.trim().length > 0));
      }
...
export function inferSchemaVersion(id: string): string {
  ...
  if (id.startsWith(TASKS_PREFIX)) {
    return id.endsWith(`/session${MARKDOWN_EXTENSION}`)
      ? "pragma.memory-session-summary/v1"
  ...
  if (id.startsWith(SESSIONS_EVIDENCE_PREFIX)) {
...
export function normalizeWritableMemoryContextId(id: string): ExpertAgentContextResult<string> {
  if (id === SUMMARY_CONTEXT_ID) {
    return error(
      "permission_denied",
      "summary.md is generated from skills and cannot be written directly.",
    );
  }
