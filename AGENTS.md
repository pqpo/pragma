...
core -> local-host
core -> cli
evaluation -> local-host
evaluation -> cli
interpreter -> local-host
interpreter -> cli
built-in-agents -> local-host
built-in-agents -> cli
memory -> local-host
memory -> cli
...
- 保存五个内置 Agent 的 `pragma/v5` DSL，以及 Pragma 的 `author-pragma-dsl` Skill。
- 分别导出 Pragma、Memory Curator、Store Revision 和 Skill Revision 的宿主端口；调用方式允许彼此独立。
