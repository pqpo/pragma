import { z } from "zod";

export const SkillRevisionRequestV2StoredSchema = z
  .object({
    schemaVersion: z.literal("pragma.skill-revision-request/v2"),
    capabilityId: z.string().uuid(),
    prompt: z.string().trim().min(1).max(50_000),
    source: z.enum(["user", "expert-reflection", "memory-learning"]),
    sourceDigest: z.string().regex(/^[a-f0-9]{64}$/u),
    provenance: z
      .object({
        executionId: z.string().uuid(),
        invocationId: z.string().uuid(),
        expertId: z.string().min(1).max(200),
        teamId: z.string().min(1).max(200).optional(),
      })
      .strict()
      .optional(),
    sourceRefs: z.array(z.unknown()).max(100).default([]),
    replayCases: z.array(z.unknown()).min(3).max(10).optional(),
    boundaryCase: z.unknown().optional(),
  })
  .strict();

export const SkillRevisionJobV2StoredSchema = z
  .object({
    schemaVersion: z.literal("pragma.skill-revision-job/v2"),
    id: z.string().uuid(),
    revision: z.number().int().positive(),
    draftId: z.string().uuid(),
    missionId: z.string().uuid().optional(),
    request: SkillRevisionRequestV2StoredSchema,
    state: z.enum([
      "editing",
      "running",
      "evaluating",
      "pending_review",
      "publishing",
      "completed",
      "rejected",
      "needs_attention",
      "superseded",
    ]),
    evaluation: z.unknown().optional(),
    publishedRevision: z.number().int().positive().optional(),
    supersededBy: z.string().uuid().optional(),
    error: z
      .object({ code: z.string().min(1).max(100), message: z.string().min(1).max(2_000) })
      .strict()
      .optional(),
    createdAt: z.string().datetime(),
    updatedAt: z.string().datetime(),
  })
  .strict();

export type SkillRevisionJobV2Stored = z.infer<typeof SkillRevisionJobV2StoredSchema>;
