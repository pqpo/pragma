...
describe("MissionDetailFragment", () => {
  it("uses the full detail width for a single expert", () => {
    ...
    expect(html).toContain("Mission execution is not available yet");
    expect(html).not.toContain("mission-team-inspector");
  });

  it("shows the expert inspector only for a team executor", () => {
    const html = renderToStaticMarkup(
      <MissionDetailFragment mission={missionFixture("expert_team")} />,
    );
...
function missionFixture(kind: "expert" | "expert_team"): Mission {
  return {
    ...
    executor: {
      kind,
      id: kind === "expert" ? "product_designer" : "delivery_team",
